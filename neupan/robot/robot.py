...
class robot:
        ...
        **kwargs,
        ...
        if kinematics == "acker":
            if self.max_speed[1] >= 1.57:
                print(
                    f"Warning: max steering angle of acker robot is {self.max_speed[1]} rad, which is larger than 1.57 rad, so it is limited to 1.57 rad"
                )
    ...
    def state_parameter_define(self):
        ...
        self.para_A_list = [
            cp.Parameter((3, 3), name="para_A_" + str(t)) for t in range(self.T)
        ]
        self.para_B_list = [
            cp.Parameter((3, 2), name="para_B_" + str(t)) for t in range(self.T)
        ]
        self.para_C_list = [
            cp.Parameter((3, 1), name="para_C_" + str(t)) for t in range(self.T)
        ]

        return (
            [self.para_s, self.para_gamma_a, self.para_gamma_b]
            + self.para_A_list
            + self.para_B_list
            + self.para_C_list
        )
    ...
    def I_cost(self, indep_dis, ro_obs):
        ...
        for t in range(self.T):
            ...
            I_dpp = (
                self.para_gamma_c[t] @ indep_t[:, t : t + 1]
                - self.para_zeta_a[t]
                - indep_dis[0, t]
            )
            I_list.append(I_dpp)
    ...
    def cal_vertices(self, vertices=None, length=None, width=None, wheelbase=None):
        ...
        assert (
            vertices_np.shape[1] >= 3
        ), "vertices must be a numpy array of shape (2, N), N >= 3"
