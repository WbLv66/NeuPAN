...
class DUNE(torch.nn.Module):
    ...
    def forward(
        ...
        self.obstacle_points = obs_points_list[
            0
        ]  # current obstacle points considered in the dune at time 0
    ...
    def cal_objective_distance(
        self, mu: torch.Tensor, p0: torch.Tensor
    ) -> torch.Tensor:
        """
        input:
    ...
    def load_model(
        self, checkpoint: Optional[str] = None, train_kwargs: Optional[dict] = None
    ):
        """
        ...
        try:
            ...
            self.model.load_state_dict(
                torch.load(self.abs_checkpoint_path, map_location=torch.device("cpu"))
            )
            ...
            if self.ask_to_train():
                ...
                if self.ask_to_continue():
                    ...
                    print(
                        "You can set the new model path to the DUNE class to use the trained model."
                    )
    ...
    def ask_to_train(self):
        ...
        while True:
            choice = input(
                "Do not find the DUNE model; Do you want to train the model now, input Y or N:"
            ).upper()
            if choice == "Y":
                return True
            elif choice == "N":
                ...
                raise FileNotFoundError(
                    "DUNE model checkpoint not found and training was declined by user."
                )
            else:
    ...
    def ask_to_continue(self):
        ...
        while True:
            choice = input(
                "Do you want to continue the case running, input Y or N:"
            ).upper()
            if choice == "Y":
                return True
            elif choice == "N":
                ...
                raise RuntimeError(
                    "User declined to continue after training DUNE model."
                )
            else:
                print("Wrong input, Please input Y or N.")
